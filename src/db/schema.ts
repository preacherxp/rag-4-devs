...
import { DEFAULT_CHAT_MODEL, DEFAULT_CHAT_PROVIDER, config } from "../config.js";
...
export const quizzes = pgTable(
  ...
  (t) => [check("quizzes_difficulty_check", sql`${t.difficulty} IN ('easy', 'medium', 'hard')`)],
);
...
export const quizQuestions = pgTable(
  ...
  (t) => [
    check("quiz_questions_correct_answer_check", sql`${t.correctAnswer} IN ('A', 'B', 'C', 'D')`),
    check(
      "quiz_questions_selected_answer_check",
      sql`${t.selectedAnswer} IS NULL OR ${t.selectedAnswer} IN ('A', 'B', 'C', 'D')`,
    ),
    unique("quiz_questions_quiz_id_question_index_key").on(t.quizId, t.questionIndex),
  ],
);
